...
// Same harness as CommitmentsBench.sol for selector-less hashers (packed 64-byte calldata).
pragma solidity ^0.8.12;

uint256 constant SNARK_SCALAR_FIELD =
  21888242871839275222246405745257275088548364400416034343698204186575808495617;

contract CommitmentsBenchPacked {
  ...
  function hashLeftRight(bytes32 _left, bytes32 _right) public view returns (bytes32 res) {
    ...
    assembly {
      let p := mload(0x40)
      mstore(p, _left)
      mstore(add(p, 32), _right)
      if iszero(staticcall(gas(), lib_, p, 64, p, 32)) {
        revert(0, 0)
      }
      res := mload(p)
    }
